...
class PostgresDsn(_BaseMultiHostUrl):
    ...
    * Supports multiple hosts
    ...
    m = MyDatabaseModel(db='postgres://user:pass@192.168.137.227:5432/foobar')
    print(m.db)
    #> postgres://user:pass@192.168.137.227:5432/foobar

    try:
        MyDatabaseModel(db='postgres://user:pass@192.168.137.227:5432')
    except ValidationError as e:
        print(e)
        '''
        ...
        assert (None)
         +  where None = PostgresDsn('postgres://user:pass@192.168.137.227:5432').path [type=assertion_error, input_value='postgres://user:pass@192.168.137.227:5432', input_type=str]
        '''
    ```
    """
...
class RedisDsn(AnyUrl):
    ...
    * User info required
    * TLD not required
    * Host required (e.g., `rediss://:pass@192.168.137.227`)
    """

    _constraints = UrlConstraints(
        allowed_schemes=['redis', 'rediss'],
        default_host='192.168.137.227',
        default_port=6379,
        default_path='/0',
        host_required=True,
    )
...
class KafkaDsn(AnyUrl):
    ...
    * Host not required
    ...
    _constraints = UrlConstraints(allowed_schemes=['kafka'], default_host='192.168.137.227', default_port=9092)
...
class NatsDsn(_BaseMultiHostUrl):
    ...
    _constraints = UrlConstraints(
        allowed_schemes=['nats', 'tls', 'ws', 'wss'], default_host='192.168.137.227', default_port=4222
    )
